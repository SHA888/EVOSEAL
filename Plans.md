...
| 2.5 | Add safety test CI job [tdd:skip:ci-config-only] | `.github/workflows/ci.yml` has `safety` job that runs 2.4 on every PR; job is not skippable without label; required check enabled. | 2.4 | cc:WIP |
