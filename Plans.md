...
| 2.7 | Add `--dry-run` mode [tdd:required] | `evoseal pipeline start --dry-run` sets `EVOSEAL_MOCK_MODE=true` + `EVOSEAL_DRY_RUN=true` (mocks respond, no actual edits to disk). Output deterministic for same seed. Documented in CLI help. Works with 2.6 test. | 2.3, 2.6 | cc:wip |
