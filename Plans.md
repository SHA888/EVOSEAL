...
| 2.10 | Add configurable token budget [tdd:required] | `max_tokens_per_run` and `max_cost_per_run` config options; loop stops gracefully when budget exhausted; warning at 80%. Validated against spec from 2.8. 2.6 integration test covers budget-exhaustion scenario. | 2.8, 2.9 | cc:done [pending-commit] |
