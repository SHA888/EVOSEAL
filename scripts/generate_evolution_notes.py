...
def generate_release_notes(version: str, metrics: Dict, changes: List[Dict]) -> str:
    ...
    if metrics.get('metrics'):
        m = metrics['metrics']
        output.append(f"- **Code Changes**: {m.get('code_size', {}).get('added', 0)} lines added, "
                    f"{m.get('code_size', {}).get('deleted', 0)} lines removed")
        output.append(f"- **Files Changed**: {m.get('code_size', {}).get('files_changed', 0)} files")
...
def main():
    ...
    if not args.since:
        try:
            args.since = subprocess.check_output(
    ...
    if not metrics and changes:
        metrics = {
            'changes': {
                'features': [c['message'] for c in changes if c['message'].lower().startswith(('feat', 'add', 'new'))],
                'fixes': [c['message'] for c in changes if c['message'].lower().startswith(('fix', 'bug', 'issue'))],
                'performance_improvements': [c['message'] for c in changes if c['message'].lower().startswith(('perf', 'optimize'))]
            }
        }
