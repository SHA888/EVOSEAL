...
def print_suggestions(suggestions: List[EditSuggestion]) -> None:
    ...
    for category, category_suggestions in by_category.items():
        ...
        for i, suggestion in enumerate(category_suggestions, 1):
            ...
            print(f"  {i}. {suggestion.operation.value.upper()}: {suggestion.explanation}")
...
def apply_suggestions_interactive(editor: SelfEditor, content: str, suggestions: List[EditSuggestion]) -> str:
    ...
    if history:
        for i, entry in enumerate(history, 1):
            ...
            if isinstance(entry, tuple) and len(entry) >= 2:
                ...
                operation_str = str(operation.value) if hasattr(operation, 'value') else str(operation)
                print(f"{i}. {operation_str}: {description}")
            elif hasattr(entry, 'operation') and hasattr(entry, 'description'):
                operation_str = str(entry.operation.value) if hasattr(entry.operation, 'value') else str(entry.operation)
