...
async def test_basic_safety_integration():
    ...
    try:
        ...
        print(f"   ✓ Checkpoint manager: {type(pipeline.safety_integration.checkpoint_manager).__name__}")
        print(f"   ✓ Rollback manager: {type(pipeline.safety_integration.rollback_manager).__name__}")
        print(f"   ✓ Regression detector: {type(pipeline.safety_integration.regression_detector).__name__}")
        ...
        print(f"   ✓ Safe evolution step completed")
...
async def test_evolution_cycle_integration():
    ...
    try:
        ...
        pipeline.safety_integration = SafetyIntegration(
            config.safety_config,
            pipeline.metrics_tracker,
            None
        )
        
        print(f"   ✓ Pipeline with safety integration created")
        ...
        if hasattr(pipeline, 'run_evolution_cycle_with_safety'):
            ...
            expected_params = ['iterations', 'enable_checkpoints', 'enable_auto_rollback']
