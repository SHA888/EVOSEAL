...
def create_test_checkpoints(checkpoint_manager: CheckpointManager, temp_dir: Path):
    ...
    for i in range(1, 4):
        ...
        (test_files_dir / f"main_v{i}.py").write_text(f"""
...
""")
        ...
        (test_files_dir / "config.json").write_text(json.dumps({
            "version": f"{i}.0",
...
def display_rollback_statistics(rollback_manager: RollbackManager):
    ...
    for i, event in enumerate(history, 1):
        print(f"{i}. {event['timestamp'][:19]} - {event['version_id']} ({event['reason']}) - {'✅' if event['success'] else '❌'}")
