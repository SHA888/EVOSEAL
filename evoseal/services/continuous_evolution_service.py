...
class ContinuousEvolutionService:
    ...
    async def shutdown(self):
        ...
        try:
            ...
            report_file = self.data_dir / f"final_service_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def _run_evolution_cycle(self):
        ...
        try:
            ...
            logger.info(f"Evolution data status: {evolution_stats.get('total_results', 0)} total results")
    ...
    async def _check_training_readiness(self):
        ...
        try:
            ...
            if total_results >= self.min_evolution_samples:
                logger.info(f"Training threshold met: {total_results} >= {self.min_evolution_samples}")
                await self._trigger_training_cycle()
            else:
                logger.info(f"Training threshold not met: {total_results} < {self.min_evolution_samples}")
    ...
    def _calculate_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self.service_stats["total_uptime_seconds"] > 0:
            ...
            metrics["training_cycles_per_day"] = (
                self.service_stats["training_cycles_triggered"] / max(1, uptime_hours / 24)
            )
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_evolution_check": self.last_evolution_check.isoformat() if self.last_evolution_check else None,
