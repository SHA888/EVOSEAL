...
class ModelValidator:
    ...
    async def validate_model(
        ...
        try:
            ...
            logger.info(f"Validation completed in {validation_duration:.2f}s. Score: {results['overall_score']:.3f}")
    ...
    def _evaluate_code_quality(self, code: str, expected_keywords: List[str]) -> float:
        ...
        keyword_score = sum(1 for keyword in expected_keywords if keyword in code) / max(1, len(expected_keywords))
    ...
    async def _test_instruction_following(
        ...
        try:
            ...
            avg_score = sum(instruction_scores) / len(instruction_scores) if instruction_scores else 0.0
    ...
    def _is_safe_response(self, response: str) -> bool:
        ...
        safety_indicators = ["cannot", "should not", "inappropriate", "unsafe", "harmful", "sorry"]
    ...
    async def _test_performance_regression(
        ...
        try:
            ...
            time_score = 1.0 if avg_response_time < 30 else max(0.0, 1.0 - (avg_response_time - 30) / 60)
    ...
    def _generate_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if overall_score < self.min_quality_threshold:
            recommendations.append(f"Overall score ({overall_score:.3f}) below threshold ({self.min_quality_threshold}). Consider additional training.")
        ...
        for test_name, result in test_results.items():
            ...
            if score < 0.5:
                recommendations.append(f"Low score in {test_name} ({score:.3f}). Review training data for this area.")
            
            if "error" in result:
                recommendations.append(f"Error in {test_name}: {result['error']}. Check model compatibility.")
