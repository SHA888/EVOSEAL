...
class TrainingManager:
    ...
    async def check_training_readiness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if training_candidates < self.min_training_samples:
                readiness["reason"] = f"Insufficient training samples: {training_candidates} < {self.min_training_samples}"
    ...
    async def prepare_training_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results:
                return {"success": False, "error": "No recent evolution results available"}
            ...
            logger.info(f"Training data prepared: {len(training_data['dataset']['examples'])} examples")
