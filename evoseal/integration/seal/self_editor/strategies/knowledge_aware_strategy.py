...
class KnowledgeAwareStrategy(EditStrategy):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self._default_strategy = DefaultEditStrategy()
    ...
    def get_relevant_context(self, content: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of relevant context entries from the KnowledgeBase.
        """
        # Simple implementation - can be enhanced with more sophisticated search
        return self.knowledge_base.search_entries(
            query=content,
            limit=self.max_context_entries
        )
    
    def _analyze_content_with_context(self, content: str, context_entries: List[Dict[str, Any]]) -> List[EditSuggestion]:
    ...
    def _analyze_line(self, line_num: int, line: str, lines: List[str], 
                      current_function: Optional[str], suggestions: List[EditSuggestion]) -> None:
        ...
        elif any(unsafe in line for unsafe in ["eval(", "exec(", "pickle.loads(", "subprocess.call(", "os.system("]):
            suggestions.append(EditSuggestion(
        ...
        elif line.strip().startswith('print(') and not any(skip in line for skip in ['# noqa', '# no-print']):
            suggestions.append(EditSuggestion(
                operation=EditOperation.REWRITE,
    ...
    def _analyze_function_definition(self, line_num: int, line: str, lines: List[str], 
                                   suggestions: List[EditSuggestion]) -> None:
    ...
    def _add_docstring_suggestion(self, line: str, func_def: str, func_name: str, 
                                suggestions: List[EditSuggestion]) -> None:
    ...
    def _analyze_functions(self, content: str, lines: List[str], 
                          suggestions: List[EditSuggestion]) -> None:
        ...
        if "def " in content and "try:" not in content and "except " not in content:
            ...
            for func_def in functions:
                func_name = func_def.split('(')[0].split()[-1]
                if func_name not in ['__init__', 'main', 'test_']:  # Skip common functions that might not need try/except
                    suggestions.append(EditSuggestion(
                        operation=EditOperation.ADD,
                        criteria=[EditCriteria.ROBUSTNESS, EditCriteria.ERROR_HANDLING],
                        original_text=content,
    ...
    def _add_context_based_suggestions(self, content: str, 
                                      context_entries: List[Dict[str, Any]],
                                      suggestions: List[EditSuggestion]) -> None:
        """Add suggestions based on knowledge base context."""
        # Check for security-related context
        security_context = [e for e in context_entries 
                          if any(tag in e.get('tags', []) 
                               for tag in ['security', 'best-practices'])]
    ...
    def evaluate(self, content: str, **kwargs: Any) -> List[EditSuggestion]:
        ...
        for suggestion in knowledge_suggestions:
            key = (suggestion.operation, suggestion.original_text, suggestion.suggested_text)
            all_suggestions[key] = suggestion
        
        # Add default suggestions, keeping higher confidence scores for duplicates
        for suggestion in default_suggestions:
            key = (suggestion.operation, suggestion.original_text, suggestion.suggested_text)
            if key not in all_suggestions or suggestion.confidence > all_suggestions[key].confidence:
                all_suggestions[key] = suggestion
