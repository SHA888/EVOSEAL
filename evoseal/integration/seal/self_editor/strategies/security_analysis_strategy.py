...
class SecurityAnalysisStrategy(BaseEditStrategy):
    ...
    RISKY_IMPORTS = {
        'pickle', 'cPickle', 'subprocess', 'os.system', 'os.popen', 'eval',
        'exec', 'execfile', 'input', 'code.interact', 'pty.spawn',
        'os.execl', 'os.execle', 'os.execlp', 'os.execlpe', 'os.execv',
        'os.execve', 'os.execvp', 'os.execvpe', 'os.popen', 'os.popen2',
        'os.popen3', 'os.popen4', 'os.startfile', 'os.system'
    }
    
    UNSAFE_FUNCTIONS = {
        'eval', 'exec', 'execfile', 'input', 'compile', 'open',
        'os.system', 'os.popen', 'subprocess.call', 'subprocess.Popen',
        'pickle.load', 'pickle.loads', 'cPickle.load', 'cPickle.loads',
        'marshal.load', 'marshal.loads', 'yaml.load', 'yaml.safe_load',
        'jsonpickle.decode', 'shelve.open', 'sqlite3.connect',
        'tempfile.mktemp', 'tempfile.mkstemp', 'tempfile.mkdtemp'
    }
    ...
    def evaluate(self, content: str, **kwargs) -> List[EditSuggestion]:
        ...
        try:
            ...
            for check_func in self.config.custom_checks:
                try:
                    custom_suggestions = check_func(content, tree=tree, **kwargs)
            ...
            suggestions.append(EditSuggestion(
                ...
                criteria=[EditCriteria.SECURITY, EditCriteria.SYNTAX],
                ...
                for name in node.names:
                    if name.name in self.RISKY_IMPORTS and not self._is_ignored(name.name):
                        suggestions.append(EditSuggestion(
                            ...
                            metadata={"issue_type": "risky_import", "import": name.name}
                        ))
                ...
                for name in node.names:
                    ...
                    if full_import in self.RISKY_IMPORTS and not self._is_ignored(full_import):
                        suggestions.append(EditSuggestion(
                            ...
                            confidence=0.9,
                            line_number=node.lineno,
                            metadata={"issue_type": "risky_import", "import": full_import}
                        ))
            ...
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                ...
                if func_name in self.UNSAFE_FUNCTIONS and not self._is_ignored(func_name):
                    suggestions.append(EditSuggestion(
                        ...
                        confidence=0.8,
                        line_number=node.lineno,
                        metadata={"issue_type": "unsafe_function", "function": func_name}
                    ))
    ...
    def _check_sql_injection(self, tree: ast.AST, content: str) -> List[EditSuggestion]:
        ...
        for node in ast.walk(tree):
            ...
            if (isinstance(node, ast.Call) and 
                ...
                node.args and 
                isinstance(node.args[0], (ast.JoinedStr, ast.BinOp, ast.Mod))):
    ...
    def _check_xss(self, tree: ast.AST, content: str) -> List[EditSuggestion]:
                    ...
                    if isinstance(stmt, ast.Return) and stmt.value:
                        if (isinstance(stmt.value, (ast.JoinedStr, ast.FormattedValue)) or
                            (isinstance(stmt.value, ast.Call) and 
                             ...
                             stmt.value.func.attr in ['format', 'format_map'])):
                            ...
                            suggestions.append(EditSuggestion(
                                ...
                                metadata={"issue_type": "xss", "context": "flask_route"}
                            ))
                            break
        ...
        for node in ast.walk(tree):
            if (isinstance(node, ast.Call) and 
                isinstance(node.func, ast.Attribute) and
                hasattr(node.func, 'attr') and
                node.func.attr in ['write', 'send', 'respond'] and
                any(isinstance(arg, (ast.JoinedStr, ast.FormattedValue)) for arg in node.args)):
    ...
    def _check_command_injection(self, tree: ast.AST, content: str) -> List[EditSuggestion]:
        ...
        for node in ast.walk(tree):
            ...
            if (isinstance(node, ast.Call) and
                ...
                node.args and 
                isinstance(node.args[0], (ast.JoinedStr, ast.BinOp, ast.Call, ast.Name))):
                
                suggestions.append(EditSuggestion(
                    operation=EditOperation.REWRITE,
                    criteria=[EditCriteria.SECURITY],
                    original_text=ast.get_source_segment(content, node),
                    suggested_text=f"# SECURITY: {ast.get_source_segment(content, node)}  # REVIEW: Potential command injection - use subprocess with shell=False",
        ...
        for node in ast.walk(tree):
            ...
            if (isinstance(node, ast.Call) and
                ...
                suggestions.append(EditSuggestion(
                    ...
                    criteria=[EditCriteria.SECURITY],
                    original_text=ast.get_source_segment(content, node),
                    suggested_text=f"# SECURITY: {ast.get_source_segment(content, node)}  # REVIEW: Validate file path before opening",
            ...
            for pattern, description in self.SECRET_PATTERNS:
                if re.search(pattern, line) and not self._is_ignored(line):
                    suggestions.append(EditSuggestion(
                        ...
                        metadata={"issue_type": "hardcoded_secret", "description": description}
                    ))
