...
class CodeStyleStrategy(BaseEditStrategy):
    ...
    def evaluate(self, content: str, **kwargs) -> List[EditSuggestion]:
        ...
        for i, line in enumerate(lines):
            ...
            if len(line) > self.max_line_length:
                suggestions.append(self._create_line_length_suggestion(
                    line, line_num, len(line)
                ))
            
            # Check for trailing whitespace
            if line.rstrip() != line:
                suggestions.append(self._create_trailing_whitespace_suggestion(
                    line, line_num
                ))
            
            # Check for mixed indentation
            if self._has_mixed_indentation(line):
                suggestions.append(self._create_mixed_indentation_suggestion(
                    line, line_num
                ))
    ...
    def _create_line_length_suggestion(self, line: str, line_num: int, length: int) -> EditSuggestion:
        """Create suggestion for line length violation."""
        return EditSuggestion(
            operation=EditOperation.REWRITE,
            criteria=[
                EditCriteria.STYLE,
                EditCriteria.READABILITY
            ],
            original_text=line,
