...
def parse_docstring(docstring: str, style: DocstringStyle = DocstringStyle.GOOGLE) -> ParsedDocstring:
...
def _parse_rest_docstring(rest: str, parsed: ParsedDocstring) -> None:
    ...
    while i < len(lines):
        ...
        if i > 0 and i + 1 < len(lines) and \
           len(lines[i-1]) > 0 and \
           set(lines[i]).issubset(set('=-~_*+#:^"\'`')) and \
           len(set(lines[i])) == 1 and \
           len(lines[i]) >= len(lines[i-1]):
            ...
            while i < len(lines) and not (lines[i].strip() and 
                                        i + 1 < len(lines) and 
                                        set(lines[i+1]).issubset(set('=-~_*+#:^"\'`')) and 
                                        len(set(lines[i+1])) == 1 and 
                                        len(lines[i+1]) >= len(lines[i])):
                content_lines.append(lines[i].strip())
                i += 1
