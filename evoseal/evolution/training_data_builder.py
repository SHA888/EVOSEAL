...
from .models import EvolutionResult, TrainingExample, ImprovementType
...
class TrainingDataBuilder:
    ...
    def _filter_high_quality_results(self, results: List[EvolutionResult]) -> List[EvolutionResult]:
        ...
        for result in results:
            if (result.success and 
                result.fitness_score >= self.min_quality_score and
                result.improvement_percentage > 10.0):  # Significant improvement
    ...
    def _filter_code_length(self, result: EvolutionResult) -> bool:
        ...
        return (5 <= orig_lines <= 100 and 
                5 <= imp_lines <= 150 and
                abs(orig_lines - imp_lines) <= 50)
    ...
    def _generate_context(self, result: EvolutionResult) -> str:
        ...
        if result.improvement_types:
            types_text = ', '.join([t.value.replace('_', ' ').title() for t in result.improvement_types])
    ...
    def _validate_example(self, example: TrainingExample) -> bool:
        ...
        # Length checks
        if (len(example.input_code) < 20 or 
            len(example.output_code) < 20 or
            len(example.input_code) > 5000 or
            len(example.output_code) > 7500):
            return False
    ...
    def _balance_examples(self, examples: List[TrainingExample]) -> List[TrainingExample]:
        ...
        for pattern, pattern_examples in pattern_groups.items():
            ...
            sorted_examples = sorted(pattern_examples, 
    ...
    def _save_alpaca_format(self, output_dir: Path, train_examples: List[TrainingExample], val_examples: List[TrainingExample]) -> Dict[str, Path]:
    ...
    def _save_chat_format(self, output_dir: Path, train_examples: List[TrainingExample], val_examples: List[TrainingExample]) -> Dict[str, Path]:
        """Save in chat format."""
    ...
    def _save_jsonl_format(self, output_dir: Path, train_examples: List[TrainingExample], val_examples: List[TrainingExample]) -> Dict[str, Path]:
