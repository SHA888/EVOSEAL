...
from .models import EvolutionResult, CodeMetrics, EvolutionStrategy, ImprovementType
...
class EvolutionDataCollector:
    ...
    async def collect_result(self, result: EvolutionResult) -> None:
        ...
        try:
            ...
            if result.success and result.fitness_score >= self.min_fitness_threshold:
                ...
                logger.debug(f"Collected successful result: {result.id} (fitness: {result.fitness_score:.3f})")
            else:
                self.failed_results.append(result)
                self.stats['failed_count'] += 1
                logger.debug(f"Collected failed result: {result.id} (fitness: {result.fitness_score:.3f})")
    ...
    def load_historical_data(self, days_back: int = 30) -> List[EvolutionResult]:
        ...
        try:
            ...
            logger.info(f"Loaded {len(historical_results)} historical results from last {days_back} days")
