...
from .models import EvolutionResult, PatternMatch, ImprovementType, EvolutionStrategy
...
class PatternAnalyzer:
    ...
    def _generate_summary(self, results: List[EvolutionResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            'timespan_days': (max(r.timestamp for r in results) - 
                            min(r.timestamp for r in results)).days
        }
    ...
    def _create_transformation_pattern(self, pattern_name: str, frequency: int, results: List[EvolutionResult]):
        """Create a transformation pattern object."""
        examples = []
        
        # Find examples of this pattern
        for result in results:
            if pattern_name in self._detect_transformation_patterns(result.original_code, result.improved_code):
        ...
        for result in results:
            try:
                ...
                if orig_ast and imp_ast:
                    # Count functions
                    orig_funcs = len([n for n in ast.walk(orig_ast) if isinstance(n, ast.FunctionDef)])
                    imp_funcs = len([n for n in ast.walk(imp_ast) if isinstance(n, ast.FunctionDef)])
                    
                    if imp_funcs > orig_funcs:
                        patterns['function_extraction'] += 1
                    
                    # Count classes
                    orig_classes = len([n for n in ast.walk(orig_ast) if isinstance(n, ast.ClassDef)])
                    ...
                    # Count imports
                    orig_imports = len([n for n in ast.walk(orig_ast) if isinstance(n, (ast.Import, ast.ImportFrom))])
                    imp_imports = len([n for n in ast.walk(imp_ast) if isinstance(n, (ast.Import, ast.ImportFrom))])
    ...
    def get_training_patterns(self) -> List[Dict[str, Any]]:
        ...
        for transformation in self.transformations:
            if (transformation.frequency >= self.min_pattern_frequency and 
                transformation.confidence >= self.min_confidence):
