...
- [x] **Release pipeline is broken** _(done 2026-08-04)_ — `pre-release.yml` and `scripts/lib/release/release.sh` referenced `scripts/auto_generate_release_notes.py` but the script lives at `scripts/lib/release/auto_generate_release_notes.py`; corrected both paths. Also removed stale `POETRY_VERSION` env var from `release.yml` (project uses uv, not Poetry).
...
| 🟠 P1    | 24    | 19   | Original safety/integration items done; +12 high-priority bugs from 2026-07-22 review (4 CI/CD pipeline fixes: workflow_run name mismatch, requirements/ path, security gate bypass, release pipeline path); signal-handler init fix; safety.yaml created; monitoring dashboard auth+CORS fix |
