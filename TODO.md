...
- [x] **T2-7. Update ADR 0001 "Current state" and operator guidance once T2-1–T2-6 land**,
...
- [x] **Add cost/token usage to the real-time dashboard** _(done 2026-08-03)_
  - Show cumulative API spend alongside evolution metrics
- [x] **Add a "generation diff" view**
...
- [x] **Add support for local models — Ollama** _(done 2026-08-19)_
  - Reduce API dependency for experimentation
  - Provider unit tests pass (`pytest tests/unit/providers/ -q`)
  - Live E2E verification against a real Ollama instance: 9 integration tests in `tests/integration/test_ollama_e2e.py` cover health check, model discovery (coder + reviewer + all roles), prompt submission, system messages, response parsing, code-block detection, and model-info consistency. All pass against Ollama with `deepseek-coder-v2:16b-lite-instruct-q8_0` + `qwen2.5-coder:7b-instruct-q6_K` installed
- [x] **Add support for local models — vLLM**
  - vLLM provider via OpenAI-compatible API (`evoseal/providers/vllm_provider.py`)
...
- [x] **Document EVOSEAL's hyperparameter space** _(done 2026-08-19)_ — enumerate all tuneable pipeline parameters (mutation rate, selection pressure, temperature, etc.) and their current values/ranges; prerequisite for PBT feasibility spike
...
- [x] **Human-in-the-loop feedback interface** _(done 2026-08-29)_ — FeedbackStore with CRUD + acceptance-rate stats; dashboard UI with approve/reject buttons and live stats; API endpoints (`/api/feedback/pending`, `/{id}/approve`, `/{id}/reject`, `/api/feedback/stats`); `EvolutionPipeline` gating via `_await_human_feedback`; `ContinuousEvolutionService` wiring; 59 unit tests across 3 test files
  - [x] Proposal store, dashboard UI, approve/reject endpoints
  - [x] Wire `FeedbackStore` into the self-modification pipeline (gate behind approval)
