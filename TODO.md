...
- [x] **Release pipeline is broken** _(done 2026-08-04)_ — `pre-release.yml` referenced `scripts/auto_generate_release_notes.py` but the script lives at `scripts/lib/release/auto_generate_release_notes.py`; corrected the path. Also removed stale `POETRY_VERSION` env var from `release.yml` (project uses uv, not Poetry).
