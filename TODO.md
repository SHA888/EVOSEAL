...
- [x] **Add support for local models (Ollama/vLLM)**
  - Reduce API dependency for experimentation
  - Ollama already fully integrated; added vLLM provider via OpenAI-compatible API (`evoseal/providers/vllm_provider.py`)
...
| 🟢 P3    | 24    | 20   | Makefile, pre-commit, Docker, ADRs, ADR refresh, CHANGELOG complete; +11 hygiene items from 2026-07-22 review; Ollama provider retry/backoff fix; local_models TTL cache; workspace prompt file conventions; how-it-works tutorial; model_fine_tuner key validation; model_fine_tuner GPU availability check; vLLM provider added |
| **Total** | **89** | **74** | |
